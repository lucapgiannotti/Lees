  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-8 py-8">
      ...
      <header className="mb-8">
        ...
        <p className="font-body-lg text-lg text-on-surface-variant mt-1">Configure parameters and estimate target gravity.</p>
      </header>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-8">
        ...
        <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6">
          <h3 className="font-headline-md text-xl border-b border-surface-container pb-3 mb-5">Batch Details</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-1">
              <label className="font-label-sm text-on-surface-variant">RECIPE / BASE (Optional)</label>
              <input 
                type="text" 
                value={recipe}
            ...
            {meadType === "Other" && (
              <div className="flex flex-col gap-1 md:col-span-2 animate-fade-in">
                <label className="font-label-sm text-on-surface-variant text-primary">SPECIFY STYLE</label>
                <input 
                  type="text" 
        ...
        <div className="bg-surface-container-low border border-outline-variant rounded-xl overflow-hidden">
          <div className="p-6 border-b border-surface-container">
            ...
            <div className="mb-6 w-full md:w-1/2 pr-0 md:pr-3">
              <label className="font-label-sm text-on-surface-variant">TOTAL TARGET VOLUME (GALLONS)</label>
              <input 
                type="number" 
                step="0.1" 
                min="0.1"
            ...
            <div className="p-5 bg-surface-container-lowest border border-outline-variant rounded-lg">
              {calcMode === "ingredients" ? (
                <div>
                  <label className="font-label-sm text-on-surface-variant">HONEY AMOUNT (LBS)</label>
                  <p className="text-sm text-on-surface-variant mb-2">Water/fluid will make up the remaining volume to reach {volumeGal} gallons.</p>
                  <input 
                    type="number" 
                    step="0.1" 
                    min="0"
                ...
                <div>
                  <label className="font-label-sm text-on-surface-variant">DESIRED TARGET ABV (%)</label>
                  <p className="text-sm text-on-surface-variant mb-2">Calculates honey needed, assuming fermentation runs perfectly dry (1.000 FG).</p>
                  <input 
                    type="number" 
                    step="0.1" 
                    min="0"
            ...
            {calcMode === "ingredients" ? (
              <>
                ...
                <div className="text-sm text-on-surface-variant md:max-w-xs leading-relaxed">
                  Calculations assume average honey (35 PPG) and a completely dry fermentation finishing at 1.000 FG.
                </div>
              </>
            ) : (
              <>
                <div className="flex gap-8 w-full md:w-auto items-center">
                  <div>
                    ...
                    <p className="font-metric-xl text-3xl text-primary">{requiredHoneyLbs} <span className="text-lg font-body-md text-on-surface-variant">lbs</span></p>
                  </div>
                </div>
                <div className="text-sm md:max-w-sm leading-relaxed bg-tertiary-container/30 border border-tertiary/20 p-3 rounded text-on-surface-variant flex gap-3">
                  ...
                  <p>
                    <strong>Precision Tip:</strong> Honey density varies. Add ~90% of this amount, mix thoroughly, and take a hydrometer reading. Slowly add the remaining honey to hit exactly <strong>{estOg}</strong> target OG.
                  </p>
                </div>
              </>
            )}
