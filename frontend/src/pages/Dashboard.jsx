...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    .then(([metricsData, batchesData]) => {
      ...
      const mappedBatches = batchesData.map(batch => {
        ...
        const latestLog = batch.logs && batch.logs.length > 0 
  ...
  if (isLoading) {
    return <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 text-on-surface-variant">Loading command center...</div>;
  }
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="font-body-lg text-body-lg text-on-surface-variant mt-1">Overview of your active operations.</p>
        </div>
        <p className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-widest bg-surface-container px-3 py-1 rounded-full w-fit">Last updated: Just now</p>
      </header>
      ...
      <section>
        <div className="flex justify-between items-center mb-6 border-b border-outline-variant pb-2">
          <h3 className="font-headline-md text-headline-md text-on-surface">Active Fermentations</h3>
          <Link to="/batches" className="font-label-sm text-label-sm text-primary hover:underline uppercase tracking-widest">VIEW ALL</Link>
        </div>
