...
const calculateMeasuredAbv = (batch) => {
  ...
  const og = readingsBeforeHoney[0] || batch.target_og || 1.000;
...
export default function Batches() {
  ...
  const [editForm, setEditForm] = useState({ 
    name: "", style: "", recipe: "", target_abv: "", 
    yield_bottles: 0, remaining_bottles: 0 
  });
  ...
  if (selectedBatch) {
    ...
    return (
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-8 relative">
        <button onClick={() => setSelectedBatch(null)} className="flex items-center gap-2 text-on-surface-variant hover:text-primary transition-colors mb-6 font-label-sm uppercase tracking-widest">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span className={`px-2 py-0.5 font-label-sm text-xs rounded ${getPhaseColor(selectedBatch.phase)}`}>{selectedBatch.phase || 'Unknown Phase'}</span>
          ...
          <div className="flex gap-3 items-center">
            <button onClick={openEditModal} className="bg-surface-container-lowest border border-outline text-on-surface px-4 py-2 rounded font-label-sm uppercase tracking-widest hover:bg-surface-container transition-colors">Edit</button>
            <div className="relative flex items-center">
              <select value={selectedBatch.phase || 'Primary'} onChange={handlePhaseChange} className="appearance-none bg-primary text-on-primary px-4 py-2 pr-10 rounded font-label-sm uppercase tracking-widest hover:opacity-90 cursor-pointer transition-opacity border border-transparent focus:outline-none focus:ring-1 focus:ring-primary">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                {isBottled ? (
                  <>
                    <div className="flex flex-col">
                      <p className="font-label-sm text-on-surface-variant mb-1 uppercase text-[10px]">Remaining</p>
                      <p className="font-metric-xl text-3xl">{selectedBatch.remaining_bottles} <span className="text-lg text-on-surface-variant font-body-md">/ {selectedBatch.yield_bottles}</span></p>
                    </div>
                    <div className="flex flex-col">
                      <p className="font-label-sm text-primary mb-1 uppercase text-[10px]">Final ABV</p>
                      <p className="font-metric-xl text-3xl text-primary">{measuredAbv}%</p>
                      <p className="text-[10px] text-on-surface-variant mt-1 italic">Target: {selectedBatch.target_abv || '--'}%</p>
                    </div>
                  </>
                ) : (
                  <>
              ...
              <form onSubmit={handleSaveLog} className="flex flex-col gap-4">
                {isBottled ? (
                  <div className="flex flex-col gap-1">
                    ...
                    <select value={logRating} onChange={e => setLogRating(e.target.value)} className="bg-surface-container-lowest border border-outline-variant rounded p-3 font-body-md focus:outline-none focus:border-primary">
                ...
                {logType === 'reading' && !isBottled ? (
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col justify-end gap-1"><label className="font-label-sm text-on-surface-variant uppercase text-[10px]">Specific Gravity</label><input type="number" step="0.001" value={logSg} onChange={e => setLogSg(e.target.value)} placeholder="1.000" className="bg-surface-container-lowest border border-outline-variant rounded p-3 font-body-md" /></div>
                    <div className="flex flex-col justify-end gap-1"><label className="font-label-sm text-on-surface-variant uppercase text-[10px]">Temp (°F)</label><input type="number" value={logTemp} onChange={e => setLogTemp(e.target.value)} placeholder="68" className="bg-surface-container-lowest border border-outline-variant rounded p-3 font-body-md" /></div>
                  </div>
                ) : !isBottled && (
                  <div className="flex flex-col gap-1"><label className="font-label-sm text-tertiary uppercase text-[10px]">Honey Added (Grams)</label><input type="number" value={logHoney} onChange={e => setLogHoney(e.target.value)} placeholder="e.g. 200" className="bg-surface-container-lowest border border-tertiary/50 rounded p-3 font-body-md focus:outline-none focus:border-tertiary focus:ring-1 focus:ring-tertiary transition-all" required/></div>
                )}
                <div className="flex flex-col gap-1">
                  <label className="font-label-sm text-on-surface-variant uppercase text-[10px]">Notes</label>
                  <textarea rows="3" value={logNote} onChange={e => setLogNote(e.target.value)} placeholder={isBottled ? "Aroma, clarity, taste..." : "Observations..."} className="bg-surface-container-lowest border border-outline-variant rounded p-3 font-body-md focus:outline-none focus:border-primary transition-all resize-none"></textarea>
                </div>
                <button type="submit" className="w-full bg-surface-container-highest border border-outline text-on-surface font-label-sm uppercase tracking-widest py-3 rounded hover:bg-surface-container transition-colors mt-2">{isBottled ? "Save Tasting & Drink 1 Bottle" : "Save Entry"}</button>
              </form>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden">
              <div className="p-6 border-b border-surface-container flex justify-between items-center"><h2 className="font-headline-md text-xl">{isBottled ? "Tasting History" : "Fermentation Log"}</h2></div>
                ...
                {logs.slice().reverse().map((log) => (
                  <div key={log.id} className="p-6 hover:bg-surface-container-low transition-colors group">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      {log.added_honey_g && <span className="font-label-sm text-tertiary border border-tertiary px-2 py-0.5 rounded uppercase tracking-widest bg-tertiary/10">+{log.added_honey_g}g Honey</span>}
                      {log.rating && <span className="font-label-sm text-primary border border-primary px-2 py-0.5 rounded uppercase tracking-widest bg-primary/10">Score: {log.rating}/5</span>}
                      {log.temp && <span className="font-body-md text-on-surface-variant flex items-center gap-1"><span className="material-symbols-outlined text-[16px]">thermostat</span>{log.temp}°F</span>}
                    </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-on-surface/20 backdrop-blur-sm p-4">
            <div className="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
              <div className="px-6 py-4 border-b border-surface-container flex justify-between items-center bg-surface-container-low">
                ...
                <button onClick={() => setIsEditModalOpen(false)} className="text-on-surface-variant hover:text-error transition-colors"><span className="material-symbols-outlined">close</span></button>
              </div>
              <form onSubmit={handleSaveEdit} className="p-6 flex flex-col gap-5 text-on-surface">
                <div className="flex flex-col gap-1"><label className="text-xs uppercase font-label-sm text-on-surface-variant">Batch Name</label><input type="text" value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} className="bg-surface-container-lowest border border-outline-variant rounded p-3 font-body-md focus:outline-none focus:border-primary" required /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1"><label className="text-xs uppercase font-label-sm text-on-surface-variant">Style</label><input type="text" value={editForm.style} onChange={e => setEditForm({...editForm, style: e.target.value})} className="bg-surface-container-lowest border border-outline-variant rounded p-3 font-body-md focus:outline-none focus:border-primary" /></div>
                  <div className="flex flex-col gap-1"><label className="text-xs uppercase font-label-sm text-on-surface-variant">Recipe</label><input type="text" value={editForm.recipe} onChange={e => setEditForm({...editForm, recipe: e.target.value})} className="bg-surface-container-lowest border border-outline-variant rounded p-3 font-body-md focus:outline-none focus:border-primary" /></div>
                </div>
                <div className="grid grid-cols-2 gap-4 border-t border-surface-container pt-4">
                  <div className="flex flex-col gap-1">
                    <label className="text-xs uppercase font-label-sm text-on-surface-variant">Total Yield (Bottles)</label>
                    <input type="number" value={editForm.yield_bottles} onChange={e => setEditForm({...editForm, yield_bottles: e.target.value, remaining_bottles: e.target.value})} className="bg-surface-container-lowest border border-outline-variant rounded p-3 font-body-md focus:outline-none focus:border-primary" />
                  </div>
                  <div className="flex flex-col gap-1">
                    <label className="text-xs uppercase font-label-sm text-on-surface-variant">Remaining</label>
                    <input type="number" value={editForm.remaining_bottles} onChange={e => setEditForm({...editForm, remaining_bottles: e.target.value})} className="bg-surface-container-lowest border border-outline-variant rounded p-3 font-body-md focus:outline-none focus:border-primary" />
                  </div>
                </div>
                <div className="mt-4 flex flex-col sm:flex-row justify-between items-center pt-4 border-t border-surface-container gap-4">
                  <button type="button" onClick={handleDeleteBatch} className="text-error font-label-sm uppercase px-4 py-2 flex items-center gap-1 hover:bg-error-container rounded transition-colors w-full sm:w-auto justify-center"><span className="material-symbols-outlined text-[18px]">delete</span> Delete Batch</button>
                  <div className="flex gap-3 w-full sm:w-auto">
                    <button type="button" onClick={() => setIsEditModalOpen(false)} className="flex-1 sm:flex-none px-6 py-3 border border-outline font-label-sm uppercase rounded hover:bg-surface-container transition-colors">Cancel</button>
                    <button type="submit" className="flex-1 sm:flex-none bg-primary text-on-primary font-label-sm uppercase px-6 py-3 rounded hover:opacity-90 transition-opacity">Save Changes</button>
                  </div>
                </div>
              </form>
            </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
      <header className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <button className="bg-primary text-on-primary px-4 py-2 rounded font-label-sm uppercase tracking-widest hover:opacity-90 transition-opacity flex items-center gap-2 w-fit"><span className="material-symbols-outlined text-sm">add</span> New</button>
      </header>
              ...
              return (
                <div key={batch.id} onClick={() => setSelectedBatch(batch)} className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 items-center hover:bg-surface-container transition-colors cursor-pointer group">
                  ...
                  <div className="col-span-2 flex items-center gap-2">
                    {activeTab === 'BOTTLED' ? <span className="font-body-md font-medium text-on-surface">{batch.remaining_bottles} bottles</span> : <><span className="font-body-md font-medium text-on-surface">{latestLog?.sg || '--'}</span><span className="text-on-surface-variant text-sm">({latestLog?.temp ? `${latestLog.temp}°F` : 'No temp'})</span></>}
                  </div>
