...
const initialInventory = [
  ...
  { id: 2, name: 'Go-Ferm', category: 'Nutrients', current: 1, desired: 4, unit: 'oz' },
  { id: 3, name: 'Fermaid O', category: 'Nutrients', current: 0, desired: 2, unit: 'oz' },
  {
    id: 4,
    name: 'Bottles (750ml)',
    category: 'Packaging',
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="font-body-lg text-lg text-on-surface-variant mt-1">
            Manage your ingredients, chemicals, and hardware.
          </p>
        </div>
        ...
        <div className="divide-y divide-surface-container">
          {filteredInventory.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 items-center hover:bg-surface-container-low transition-colors"
              >
                <div className="col-span-4 flex flex-col items-start gap-1">
                  <p className="font-headline-md text-lg text-on-surface">{item.name}</p>
                  <span
                    className={`px-2 py-0.5 font-label-sm text-[10px] uppercase rounded ${getStatusColor(item.current, item.desired)}`}
                  >
                    {item.current === 0
                      ? 'Out of Stock'
                      : item.current <= item.desired * 0.3
                        ? 'Low Stock'
                        : 'In Stock'}
                  </span>
                </div>
                ...
                <div className="col-span-3 flex items-center justify-end gap-3 mt-4 md:mt-0">
                  ...
                  <div className="bg-surface-container border border-outline-variant rounded flex items-center overflow-hidden">
                    ...
                    <span className="font-body-md font-medium px-2 min-w-[3ch] text-center">
                      {item.current}
                    </span>
                    <button
                      onClick={() => adjustStock(item.id, 1)}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingItem({ ...editingItem, desired: parseInt(e.target.value) || 0 })
                    }
                    className="bg-surface-container-lowest border border-outline-variant rounded p-3 font-body-md focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"
                    required
                  />
